...
class TradeRequestRepository(ITradeRequestRepository):
    ...
    async def show_user_requests(
        ...
        res = await session.execute(
            select(TradeRequestOrm).where(
                (TradeRequestOrm.receiver_id == owner_id)
                | (TradeRequestOrm.sender_id == owner_id)
            )
        )
        return res.scalars().all()

    async def show_user_senders(
        self, session: AsyncSession, owner_id: UUID
    ) -> list[TradeRequestOrm]:
