...
class TransactionService(ITransactionService):
    ...
    async def list_for_user(
        ...
        async with session.begin():
            ...
            return await self._transaction_repository.show_user_transactions(
                session, owner_id
            )
    ...
    async def graph_portfolio_val(
        self, owner_id: UUID, days: int, session: AsyncSession
    ):
        ...
        async with session.begin():
            ...
            if days <= 0:
                raise ValueError(
                    "Number of days cant be 0 or less"
                )
            ...
            if portfolio is None:
                raise ValueError(
                    "portfolio of given user is None"
                )
            ...
            non_tether_coins = [coin for coin in portfolio.coins if coin.lower() != "tether"]
            if not non_tether_coins:
                raise ValueError("No graphable coins in portfolio")


            for crypto in portfolio.coins:
                if crypto == "tether":
                    continue
                transactions = await self._transaction_repository.show_user_transactions_between_date_by_coin(
                    session, now - timedelta(days=days_back), now, owner_id, crypto
                )
                ...
                portfolio_start_quant = (
                    portfolio_quant - portfolio_transactions_quant
                )  # 0.2
                    ...
                    if timestamp == timestamps[0]:
                        if sorted_transactions[0].date <= timestamp:
                            prices_final.append(
                                ...
                                * (
                                    portfolio_start_quant
                                    + sorted_transactions[0].quantity
                                )
                            )
            ...
            legend = {
                k: round(v, 3)
                for k, v in portfolio.coins.items()
                if k.lower() != "tether"
            }
    ...
    async def graph_multiple_coins(
        ...
        """The method for generating a graph showing the portfolio value up to a year backwards separated by each coin in portfolio.
        ...
        if days <= 0:
            raise ValueError("Number of days cant be 0 or less")
    ...
    async def graph_p_n_l_percent(self, owner_id: UUID, session: AsyncSession) -> None:
        ...
        async with session.begin():
            ...
            transactions_general = (
                await self._transaction_repository.show_user_transactions(
                    session, owner_id
                )
            )

            sorted_transactions = sorted(
                [t for t in transactions_general if t.bought_price > 0],
                key=lambda x: x.date,
            )
            ...
            if not sorted_transactions:
                raise HTTPException(
                    status_code=404,
                    detail="No purchase transactions found for the user",
                )
            ...
            p_n_ls_whole = np.zeros(
                (len(sorted_transactions), len(price_usd)), dtype=float
            )
            p_n_ls_whole_pos = 0
            timestamps_oldest = []
            for coin in sorted_transactions:
                ...
                if len(timestamps_oldest) <= len(p_n_ls_whole[p_n_ls_whole_pos]):
                    p_n_ls_whole[p_n_ls_whole_pos] = p_n_ls_whole[p_n_ls_whole_pos][
                        : len(timestamps_oldest)
                    ]

                if len(timestamps_oldest) > len(p_n_ls_whole[p_n_ls_whole_pos]):
                    timestamps_oldest = timestamps_oldest[
                        : len(p_n_ls_whole[p_n_ls_whole_pos])
                    ]
    ...
    async def graph_p_n_l(self, owner_id: UUID, session: AsyncSession):
        ...
        async with session.begin():
            transactions = await self._transaction_repository.show_user_transactions(
                session, owner_id
            )
            now = datetime.now(timezone.utc)

            sorted_transactions = sorted(
                [t for t in transactions if t.bought_price > 0], key=lambda x: x.date
            )
            ...
            for coin in sorted_transactions:
                ...
                for i in prices:
                    p_n_ls.append(
                        (i * coin.quantity) - (coin.bought_price * coin.quantity)
                    )
