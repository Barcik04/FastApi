...
class TradeRequestService(ITradeRequestService):
    ...
    _trade_request_repository: ITradeRequestRepository
    _portfolio_repository: IPortfolioRepository

    def __init__(
        self,
        trade_request_repository: ITradeRequestRepository,
        portfolio_repository: IPortfolioRepository,
    ) -> None:
        """The initializer of the `trade request service`.

        Args:
            trade_request_repository (ITradeRequestRepository): The reference to the trade request repository.
            portfolio_repository (IPortfolioRepository): The reference to the portfolio repository.
        """
        self._trade_request_repository = trade_request_repository
        self._portfolio_repository = portfolio_repository

    async def show_user_requests(
        self, owner_id: UUID, session: AsyncSession
    ) -> list[TradeRequestOrm]:
        ...
        async with session.begin():
            portfolio = await self._portfolio_repository.show_user_portfolio(
                session, owner_id
            )
            ...
            requests = await self._trade_request_repository.show_user_requests(
    ...
    async def create_user_request(
        self, body: TradeRequestIn, owner_id: UUID, session: AsyncSession
    ) -> str:
        """The method creates a trade request aimed to a particular user specified in body with coin and quantity.

        Args:
            owner_id (UUID): The id of the user.
            body (TradeRequestIn): The body DTO of the trade request contains:
                (coin: str, quantity: float, coin_get: str, quantity_get: float, receiver_id: UUID)
            session (AsyncSession): database session.
        ...
        async with session.begin():
            ...
            portfolio_receiver = await self._portfolio_repository.find_portfolio_by_id(
                session, body.receiver_id
            )

            if portfolio_receiver is None:
                raise ValueError(f"Couldnt find portfolio with given id: {body.receiver_id}")

            if portfolio.id == portfolio_receiver.id:
                raise ValueError("You cannot send coins to the same portfolio you are sending from")

            if not portfolio.coins.get(body.coin):
                raise ValueError(f"There is no coin with that name in your portfolio: {body.coin}")

            if portfolio.coins.get(body.coin) < body.quantity:
                raise ValueError(f"There is not enough quantity: {body.quantity} of coin in your portfolio: {body.coin}")
    ...
    async def _proceed_trade(
        self, session: AsyncSession, request, sender_portfolio, receiver_portfolio
    ) -> None:
        ...
        sender_coins[request.coin] = (
            sender_coins.get(request.coin, 0.0) - request.quantity
        )
        sender_coins[request.coin_get] = (
            sender_coins.get(request.coin_get, 0.0) + request.quantity_get
        )

        receiver_coins[request.coin] = (
            receiver_coins.get(request.coin, 0.0) + request.quantity
        )
        receiver_coins[request.coin_get] = (
            receiver_coins.get(request.coin_get, 0.0) - request.quantity_get
        )
        ...
        if request.coin_get not in sender_bought:
            sender_bought[request.coin_get] = sender_bought.get(
                request.coin_get, price_usd_get
            )
    ...
    async def update_user_request(
        self, owner_id: UUID, accept: bool, request_id: UUID, session: AsyncSession
    ) -> str:
        ...
        async with session.begin():
            request = await self._trade_request_repository.find_request(
                session, request_id
            )

            sender_portfolio = await self._portfolio_repository.find_portfolio_by_id(
                session, request.sender_id
            )
            receiver_portfolio = await self._portfolio_repository.show_user_portfolio(
                session, owner_id
            )

            if request.receiver_id != receiver_portfolio.id and accept:
                raise ValueError("This method was sent by you so you can only reject it")

            if request.status in (TradeStatus.REJECTED, TradeStatus.COMPLETED):
                raise ValueError("This trade has already been rejected or completed")
